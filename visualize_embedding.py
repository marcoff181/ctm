...
def plot_full_overview(image, watermarked, embedded_blocks, detected_blocks, save_path=None):
    ...
    for loc in embedded_blocks:
        rect = plt.Rectangle((loc[1], loc[0]), BLOCK_SIZE, BLOCK_SIZE, linewidth=2.5, edgecolor='blue', facecolor='none')
        axes[0, 1].add_patch(rect)
    for loc in detected_blocks:
        color = 'orange'
        if loc in embedded_blocks:
            color = 'green'
        rect = plt.Rectangle((loc[1], loc[0]), BLOCK_SIZE, BLOCK_SIZE, linewidth=2.5, edgecolor=color, facecolor='none')
        axes[0, 1].add_patch(rect)
    axes[0, 1].set_title('Block Overview\nEmbedded (blue), Detected (orange), Both (green)', fontsize=15, fontweight='bold')
    axes[0, 1].axis('off')
    legend_elements = [
        Patch(edgecolor='blue', facecolor='none', label='Embedded Block', linewidth=2.5),
        Patch(edgecolor='orange', facecolor='none', label='Detected Block', linewidth=2.5),
        Patch(edgecolor='green', facecolor='none', label='Embedded & Detected', linewidth=2.5)
    ]
    axes[0, 1].legend(handles=legend_elements, loc='upper right', fontsize=12, frameon=True)
    ...
    for loc in embedded_blocks:
        rect = plt.Rectangle((loc[1], loc[0]), BLOCK_SIZE, BLOCK_SIZE, linewidth=2, edgecolor='blue', facecolor='none')
        axes[1, 0].add_patch(rect)
    for loc in detected_blocks:
        rect = plt.Rectangle((loc[1], loc[0]), BLOCK_SIZE, BLOCK_SIZE, linewidth=2, edgecolor='orange', facecolor='none')
        axes[1, 0].add_patch(rect)

    # Bottom right: Singular value change heatmap
    im2 = axes[1, 1].imshow(sv_changes, cmap='coolwarm')
    axes[1, 1].set_title('Singular Value Change (LL[0])', fontsize=15, fontweight='bold')
    axes[1, 1].axis('off')
    cbar2 = plt.colorbar(im2, ax=axes[1, 1], fraction=0.046, pad=0.04)
    cbar2.set_label('ΔS₀ (LL)', fontsize=12, fontweight='bold')

    fig.suptitle('Watermark Embedding & Detection: Full Overview', fontsize=18, fontweight='bold', y=0.98)
    if save_path:
...
def main(image_path, watermarked_path):
    ...
    plot_full_overview(image, watermarked, embedded_blocks, detected_blocks, save_path="full_embedding_detection_overview.png")
