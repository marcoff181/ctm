...
def extraction(watermarked_image):

    # move from a matrix to an array
    watermarked = watermarked_image.flatten()
    # Create empy flatten watermark array
    extracted_watermarked = np.zeros(WATERMARK_SIZE)
    for i in range(WATERMARK_SIZE):
        extracted_watermarked[i] = int(format(watermarked[i], "08b")[-1], 2)
        # [-1] locate the pixels LSB (i.e. the last position). If you embed the watermark in another position, change the value inside [] accordingly

    return extracted_watermarked
...
def detection(original_path, watermarked_path, attacked_path):
    ...
    # original_image = imread(original_path, 0).copy()
    watermarked_image = imread(watermarked_path, 0).copy()
    attacked_image = imread(attacked_path, 0).copy()

    original_watermark = extraction(watermarked_image)
    watermark_extracted = extraction(attacked_image)
