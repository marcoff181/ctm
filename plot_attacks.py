...
def study_attack_parameter_ranges(images_dir="./images/", num_images=1, alpha_steps=20):
    ...
    if not images:
        print("Error: Could not load any images. Please check the image paths and files.")
        return
    ...
    for i, (attack_name, wpsnr_values) in enumerate(results.items()):
        ax = axes[i]
        ax.plot(alpha_values, wpsnr_values, marker='.', linestyle='-', label='Avg. WPSNR')
        
        # Plot the critical threshold line
        ax.axhline(y=attacked_wpsnr_lower_bound, color='r', linestyle='--', 
...
def visualize_attack_combination(original, watermarked, detection, grid_size=20):
    ...
    for p1 in params1:
        ...
        for p2 in params2:
            ...
            u1.append(p1); u2.append(p2); uwpsnr.append(wpsnr)

    vmin, vmax = min(uwpsnr), max(uwpsnr)

    fig, ax = plt.subplots(figsize=(10, 8))
    if dwpsnr:
        ax.scatter(d1, d2, c=dwpsnr, cmap='Reds', s=70, alpha=0.7, marker='x',
                   vmin=vmin, vmax=vmax, edgecolors='darkred', linewidths=1.2, label='Detected')
    if uwpsnr:
        sc = ax.scatter(u1, u2, c=uwpsnr, cmap='RdYlGn_r', s=90, alpha=0.8, marker='o',
                        vmin=vmin, vmax=vmax, edgecolors='black', linewidths=0.5, label='Undetected')
    ...
    ax.set_title(f'{attack1_name} + {attack2_name} Attack Grid ({grid_size}×{grid_size})')
